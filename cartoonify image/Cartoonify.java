...
public class Cartoonify {
	...
	public void gaussianBlur() {
		...
		int[] newPixels = new int[width * height];
		int[] sum;
		int[] Pixels=pixels[currImage];
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				sum=convolution(x, y, GAUSSIAN_FILTER,5,3,width,height,Pixels);
				int red = clamp(sum[RED] / GAUSSIAN_SUM);
				int green = clamp(sum[GREEN] / GAUSSIAN_SUM);
				int blue = clamp(sum[BLUE] / GAUSSIAN_SUM);
	...
	public void sobelEdgeDetect() {
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				int[] vertical = convolution(x, y, SOBEL_VERTICAL_FILTER,3,2,width,height,Pixels);
				int[] horizontal = convolution(x, y, SOBEL_HORIZONTAL_FILTER,3,2,width,height,Pixels);
				int verticalGradient = Math.abs(vertical[RED]) + Math.abs(vertical[GREEN]) + Math.abs(vertical[BLUE]);
				int horizontalGradient = Math.abs(horizontal[RED]) + Math.abs(horizontal[GREEN]) + Math.abs(horizontal[BLUE]);
	 ...
	 */
	int[] convolution(int xCentre, int yCentre, int[] filter,int filterSize,int filterHalf,int width, int height,int[] Pixels) {
		int[] sum =new int[3];
...
//		}
	...
	protected void processPhotoOpenCL() {
		...
		int input[] = currentImage();// Array 'seeds' stores the seed number
		int output[] = new int[width*height];// Array 'output' stores the total number of dart inside the circle


		////////////////////////////////////////////////////////////////////////////////////////////////////
		// The below is the OpenCL-related code
		////////////////////////////////////////////////////////////////////////////////////////////////////
		Pointer ptrInput = Pointer.to(input);
		...
		clSetKernelArg(kernel, 0, Sizeof.cl_mem, Pointer.to(memIn1)); // Array 'input'
		clSetKernelArg(kernel, 1, Sizeof.cl_int, Pointer.to(new int[] { width }));// 'width'
		clSetKernelArg(kernel, 2, Sizeof.cl_int, Pointer.to(new int[] { height }));// 'height'
		clSetKernelArg(kernel, 3, Sizeof.cl_mem, Pointer.to(memOut));// Array 'output'

		// Set the work-item dimensions
		long global_work_size[] = new long[] { width,height }; // Global work group size is the number of repeats
		long local_work_size[] = new long[] { wgSize,wgSize };
